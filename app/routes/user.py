...
用户路由蓝图
"""
from flask import Blueprint, render_template, request, jsonify, session, redirect, current_app, send_from_directory
...
@user_bp.route('/api/profile/password', methods=['POST'])
def change_password():
    """修改密码"""
    if not session.get('user_id'):
        return jsonify({'status': 'unauthorized', 'message': '请先登录'}), 401
...
@user_bp.route('/api/stats/by_subject')
...
@user_bp.route('/api/stats/by_type')
def stats_by_type():
    """按题型统计答题情况"""
    if not session.get('user_id'):
        return jsonify({'status': 'unauthorized', 'message': '请先登录'}), 401
...
@user_bp.route('/api/profile/avatar', methods=['POST'])
def upload_avatar():
    """上传用户头像"""
    if not session.get('user_id'):
        return jsonify({'status': 'unauthorized', 'message': '请先登录'}), 401
